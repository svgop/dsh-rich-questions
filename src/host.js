...
const ANNOUNCEMENT_EN = [
  ...
  "WRITING STANDARD (every user-facing string: intro, prompts, labels, descriptions, insights) — write for a reader with zero context: plain technical English, complete short sentences with subject and verb, active voice, one fact per sentence, the point first. A label states a choice plainly; a description is one full sentence saying what choosing it means; an insight row is one complete, checkable sentence — the label organizes it, the sentence carries it. Spell a term out where you name it; numbers and file paths carry the weight. Exemplars — prompt: “The grace period (the 30-second window after a deploy where errors are ignored, set in deploy.yaml) may be too short for cold starts. How long should it be?” · description: “Choosing this schedules automatic token rotation monthly instead of leaving it manual.” · insight row: “**Proven at:** Linear rotates tokens this way in production and reported fewer token-theft incidents.”",
  ...
  "RECORDS — three surfaces: in-progress drafts at <workspace>/.dsh/survey-drafts/ (the builder loop works there); every SETTLED survey lands in the machine store survey_records reads AND as a dated copy at <workspace>/.dsh/surveys/ — when entering a project, list that lane and survey_records first: settled stances are the project’s decision history, and re-asking a settled question says nobody was listening.",
...
const ANNOUNCEMENT_ZH = [
  ...
  "写作标准（一切用户可见文案：导语、题干、标签、描述、洞察）——为毫无上下文的读者而写：平实的技术英语语体（中文场景用平实中文），完整短句，主语谓语齐全，主动语态，一句一个事实，重点在前。标签把选择说清楚；描述用一句完整的话说明选了它意味着什么；洞察行是一句完整可核查的句子——标签负责组织，句子负责表达。术语首次出现就地解释；让数字和文件路径承担分量。示例——题干：「宽限期（部署后 30 秒内错误被忽略的窗口，设在 deploy.yaml）对冷启动可能太短。应该设多长？」· 描述：「选择此项将把令牌轮换改为每月自动执行，而不是手动。」· 洞察行：「**Proven at:** Linear 在生产环境按此方式轮换令牌，并报告令牌失窃事件减少。」",
  ...
  "记录——三处可读：进行中草稿在 <workspace>/.dsh/survey-drafts/（构建器循环的工作目录）；每份已答完的问卷除写入 survey_records 读取的本机存档外，还以日期命名落在 <workspace>/.dsh/surveys/——进入一个项目时先列该目录并查 survey_records：已定立的立场就是项目的决策史，重问已答过的问题等于没人在听。",
].join("\n\n")
...
function surveyToolDefinition(ctx, service) {
  const optionSchema = {
    ...
    properties: {
      ...
      label: { type: 'string', description: 'Short user-facing option label: the choice stated plainly, readable by someone with zero context (e.g. "Rotate tokens automatically every month").' },
      description: { type: 'string', description: 'One full sentence shown under the label (always visible): subject and verb present, saying plainly what choosing this option means.' },
      insight: { type: 'string', description: 'Markdown revealed on hover (~6 lines), 3-5 markdown ROWS, one specific checkable point per row, each led by a content label (Pattern / Proven at / Breaks when / Here now / Evidence — labels vary with the content). Every row is one COMPLETE sentence a stranger can verify — the label organizes it, the sentence carries it (e.g. "**Proven at:** Linear rotates tokens this way in production and reported fewer token-theft incidents."). Minimum: one row names a real-world proven use (system/product/repo + outcome); one row states the tradeoff or break condition; one row gives the current-state handle (file path / number / record).' },
